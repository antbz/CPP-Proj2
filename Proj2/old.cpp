...
//
// General input and display functions
//
...
void extractDate(Date &date, bool file = false) {
    // Checks if date.full is a proper date and extracts that information to the rest of the vars
    // file variable specifies the context the function is called in, if reading from a file, date
    // order will be YYYY/MM/DD, however, to make the program more user-friendly for users used to
    // DD/MM/YYYY, when called in the context of reading from user input (default), it checks for
    // that format. Also checks if date is a real, does not detect leap years (29/02 always exists)
    try {
...
void extractAddress(Address &address, bool file = true) {
    // Checks if address in file_format/display_format is valid and extracts it to the
    // rest of the variables. The test for validity is only that it must have 5 elements
    vector<string> temp;
    if (file) {
...
void writeAgency(Agency &agency) {
...
void readClients(vector<Client> &clients, const string &f_name) {
...
void writeClients(vector<Client> &clients, const string &f_name) {
...
int findClient(vector<Client> &clients, const string &nif) {
    for (int i = 0; i < clients.size(); i++) {
...
int editClient(vector<Client> &clients, const string &nif) {
...
int deleteClient(vector<Client> &clients, const string &nif) {
    int pos = findClient(clients, nif);
    if (pos == -1) {
        cerr << "Cliente não encontrado!";
        return -1;
...
void readPacks(vector<Pack> &p, const string &f_name) {
...
void writePacks(vector<Pack> &packs, const string &f_name) {
    ofstream packs_file(f_name);
    packs_file << lastID << endl;

    for (int i = 0; i < packs.size(); i++) {
        if (packs[i].soldSeats == packs[i].totalSeats)
        ...
        if (packs[i].destinations.size() > 1) {
            ...
            packs_file << endl;
        packs_file << setfill('0') << setw(4) << packs[i].startDate.year << '/' << setw(2) << packs[i].startDate.month
                   << '/' << setw(2) << packs[i].startDate.day << endl;
        packs_file << setfill('0') << setw(4) << packs[i].endDate.year << '/' << setw(2) << packs[i].endDate.month
                   << '/' << setw(2) << packs[i].endDate.day << endl << setfill(' ');
...
void showPackVect(vector<Pack> &v, vector<int> range, const string &message) {
...
vector<int> findPacks(vector<Pack> &packs, string &dest) {
    vector<int> result;
    for (int i = 0; i < packs.size(); i++) {
        if (find(packs.at(i).destinations.begin(), packs.at(i).destinations.end(), dest) !=
            packs.at(i).destinations.end())
            result.push_back(i);
    }
    return result;
}
...
vector<int> findPacks(vector<Pack> &packs, const string &dest, Date &date_1, Date &date_2) {
    vector<int> result;
    for (int i = 0; i < packs.size(); i++) {
        if (find(packs.at(i).destinations.begin(), packs.at(i).destinations.end(), dest) !=
            packs.at(i).destinations.end()
...
int editPack(vector<Pack> &packs, int id) {
    ...
    while (true) {
        cout << setw(4) << left << '|' << "Data de início (" << packs.at(pos).startDate.day << "/"
             << packs.at(pos).startDate.month << "/" << packs.at(pos).startDate.year << "): ";
        getline(cin, str);
        Date tempDate;
        tempDate.full = str;
        if (str.empty()) {
    ...
    while (true) {
        cout << setw(4) << left << '|' << "Data de fim (" << packs.at(pos).endDate.day << "/"
             << packs.at(pos).endDate.month << "/" << packs.at(pos).endDate.year << "): ";
        getline(cin, str);
        Date tempDate;
        tempDate.full = str;
        if (str.empty()) {
        ...
        switch (opt) {
            ...
            case 4: {// Show packs by destination
                ...
                if (!selected.empty())
                    ...
                    cout << "Nenhum pacote com o destino " << dest << " entre as datas " << date_1.full << " e "
                         << date_2.full;
                cout << endl << "ENTER para voltar atrás";
                getline(cin, str);
                viewPacksMenu(agency, clients, packs);
